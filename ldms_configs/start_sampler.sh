...
ldmsd -a ovis -A conf=${LDMS_AUTH_FILE} -m 10MB -x sock:${LDMS_PORT} -l ${LOG_OUTPUT_DIR}/$(hostname)_sampled_$(whoami).log -s ${LOG_OUTPUT_DIR}/$(hostname)_sampled_$(whoami).sock -r ${LOG_OUTPUT_DIR}/$(hostname)_sampled_$(whoami).pid -c /dev/shm/sampler.conf -v DEBUG
